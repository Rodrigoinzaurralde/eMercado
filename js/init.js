...
async function consultarUbicacionFallback() {
  try {
    console.log(
      "Intentando API de respaldo ipapi.co para obtener ubicación..."
    );
    mostrarNotificacionUbicacion(
      "Detectando ubicación con API de respaldo...",
      "info"
    );
    ...
    console.log(
      `Ubicación actualizada desde respaldo ${data.city}, ${data.region}, ${data.country_name}`
    );

    actualizarMensajeEnvio(data.city?.toLowerCase() || "");

    mostrarNotificacionUbicacion(
      `Ubicación detectada: ${data.city}, ${data.country_name}`,
      "success"
    );
  } catch (error) {
    ...
    mostrarNotificacionUbicacion(
      "No se pudo detectar la ubicación. Usando ubicación por defecto: Montevideo, Uruguay",
      "warning"
    );
  }
}
...
function mostrarNotificacionUbicacion(mensaje, tipo = "info") {
  const notificacionAnterior = document.getElementById(
...
window.addEventListener("DOMContentLoaded", () => {
  if (
    ...
    if (apiError === "true") {
      ...
      mostrarNotificacionUbicacion(
        apiErrorMessage || "Error en la API de ubicación principal",
        "warning"
      );
      ...
      console.warn(
        "No se encontró información de ubicación. Intentando obtener ubicación..."
      );

      const notificacionMostrada = sessionStorage.getItem(
        "ubicacion_notificacion_mostrada"
      );
      if (!notificacionMostrada) {
        mostrarNotificacionUbicacion(
          "Detectando ubicación para calcular costos de envío...",
          "info"
        );
        sessionStorage.setItem("ubicacion_notificacion_mostrada", "true");
      }
      ...
      if (!notificacionMostrada) {
        ...
        console.log(
          `Ubicación detectada correctamente: ${nombreCiudad}${nombreRegion}`
        );
        mostrarNotificacionUbicacion(
          `Ubicación: ${nombreCiudad}${nombreRegion}`,
          "success"
        );
    ...
    if (ciudad === "montevideo" || ciudad === "canelones") {
      envioMsg =
        '<div class="envio-msg">Envíos gratis en Montevideo y Canelones</div>';
    } else if (ciudad === "maldonado" || ciudad === "rocha") {
      envioMsg =
        '<div class="envio-msg">Envíos a partir de 500 pesos uruguayos</div>';
    } else if (
      ciudad === "rivera" ||
      ciudad === "artigas" ||
      ciudad === "punta del este"
      ...
      envioMsg =
        '<div class="envio-msg">Envíos a partir de 700 pesos uruguayos</div>';
    } else {
      envioMsg =
        '<div class="envio-msg">Consulte por costo de envíos en su zona</div>';
    }
  ...
  function buscadorMobile() {
    //Función para asignar id al buscador mobile
  ...
  function isMobile() {
    return (
      /Mobi|iPhone|BlackBerry|iPad|iPod|Android/i.test(navigator.userAgent) ||
...
const now = Date.now();
let marcasDeTiempo = JSON.parse(
  localStorage.getItem("bloqueoDeVisitas") || "[]"
);
